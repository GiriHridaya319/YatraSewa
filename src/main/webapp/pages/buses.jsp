<%@page import="Utils.StringUtils"%>
<%@page import="model.BusModel"%>
<%@page import="java.util.ArrayList"%>

<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
    pageEncoding="ISO-8859-1"%>
    <%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c" %>
    
    <%
	//Get the session and request objects
	HttpSession userSession = request.getSession();
	String currentUserType = (String) userSession.getAttribute("type");
	String currentUser = (String) userSession.getAttribute("username");
    String contextPath = request.getContextPath();
%>  
    
<!DOCTYPE html>
<html>
<head>
<meta charset="ISO-8859-1">
<title>Insert title here</title>
<link rel="stylesheet" href="${pageContext.request.contextPath}/stylesheets/buses.css">
</head>
<body>

<jsp:include page="<%=StringUtils.PAGE_URL_HEADER%>" />
<section class="search">
	    <!-- search bar starts -->
	    <div class="search-bar">
	        <!-- Dropdown start -->
	        <div class="dropdown">
	          <div id="drop-text" class="dropdown-text">
	            <span id="span">Bus Name</span>
	          </div>
	          <ul id="list" class="dropdown-list">
	            <li class="dropdown-list-item">Bus Name</li>

	            
	          </ul>
	        </div>
	        <!-- Dropdown ends -->
	  
	        <!-- Search box input start -->
	        <div class="search-box">
	          <form>
		          <input type="text" id="search-input" placeholder="Search by Bus Name..." name="search"/>
		          <button type="submit" style="height: 30px; margin-right: 11px;"><img src="<%=contextPath%>/resources/other/search.png"></button>
		          <a href="<%=contextPath + StringUtils.SERVLET_URL_BUSES%>"><img src="<%=contextPath%>/resources/other/clear.png" style="height: 30px;"></a>  
	          </form>
	        </div>
	        
	         <!-- Search box input ends -->
	    </div>
	    <!-- search bar ends -->
	    
	</section>
	
	<!-- Filter panel -->
<div class="filter-panel">
    <form action="${pageContext.request.contextPath}/BusesServlet" method="get">
        <div class="filter-section">
            <h3>Filter Buses</h3>
            <div class="filter-row">
                <div class="filter-item">
                    <label for="pickup">Pickup Location:</label>
                    <input type="text" id="pickup" name="pickup" placeholder="Enter pickup location" value="${param.pickup}">
                </div>
                <div class="filter-item">
                    <label for="destination">Destination:</label>
                    <input type="text" id="destination" name="destination" placeholder="Enter destination" value="${param.destination}">
                </div>
            </div>
            <div class="filter-row">
                <div class="filter-item">
                    <label for="minPrice">Min Price (NPR):</label>
                    <input type="number" id="minPrice" name="minPrice" placeholder="Min price" value="${param.minPrice}">
                </div>
                <div class="filter-item">
                    <label for="maxPrice">Max Price (NPR):</label>
                    <input type="number" id="maxPrice" name="maxPrice" placeholder="Max price" value="${param.maxPrice}">
                </div>
            </div>
            <div class="filter-buttons">
                <button type="submit" class="filter-btn">Apply Filters</button>
                <a href="${pageContext.request.contextPath}/buses" class="clear-btn">Clear Filters</a>
            </div>
        </div>
    </form>
</div>

   
        <h1 class="heading">
        <%
            // Conditionally set the services header based on user type
            if (currentUserType == "agent") {
                out.print("My Buses");
            } else {
                out.print("ALL BUSES AVAILABLE");
            }
        %>
        </h1>
        
        			

	        <!-- All busses details  are displayed. (Only for users) -->
	        <c:if test="${isUser}">
	        	<c:if test="${empty busLists}">
					<p>No buses information found.</p>
				</c:if>
			
	        	<c:forEach var="bus" items="${busLists}">
	        	<c:if test="${bus.status == 'APPROVED'}">
		<div class="card">
        	<div class="bus-info">
            	<div class="bus-details">
                	<h2>${bus.busName}</h2>
                	<p>${bus.description}</p>
            <br>
            <br>
                <b><h2>Date : ${bus.date} </h2></b>
            </div>
            <div class="timing">
                <h4>${bus.departureTime}</h4>
                <p>|</p>
                <p>|</p>
                <p>|</p>
                <h4>${bus.reachingTime}</h4>
            </div>
            <div class="route">
                <p>${bus.pickup} </p>
                <p>|</p>
                <p>|</p>
                <p>|</p>
                
                <p>${bus.destination} </p>
            </div>
            <div class="extras">
                <div class="price">
                    <h4>NPR:${bus.price}</h4>
                    
                </div>
                
            </div>
        </div>

        <div class="agent">
            <div class="agentname">
                <c:forEach var="agent" items="${AgentMap}">
       <c:if test="${bus.agentUsername eq agent.value.agentID}">
       
           <div>
               <h3>${agent.value.agentPhoneNumber}</h3>
                   <p>${agent.value.agentFirstName}</p>
                   <p>${agent.value.agentLastName}</p>
           </div>
       </c:if>
   </c:forEach>
   
            </div>
            
        </div>
        
       <form id="viewSeatsForm-${bus.busID}" action="${pageContext.request.contextPath}/SeatSelection" method="post">
    <input type="hidden" name="busId" value="${bus.busID}">
    <button type="submit" class="view-seats-btn">View Seats</button>
</form>


        
        
        
        
        
    </div>
    </c:if>
				</c:forEach>
	        </c:if>

			<!-- Only their bus information are displayed. (Only for agent) -->
			<c:if test="${isAgent}">
				<c:if test="${empty agentBusesLists}">
					<p>You have not added any buses.</p>
				</c:if>
	        	<c:forEach var="bus" items="${agentBusesLists}">
					<div class="card">
					<div class="bus-info">
					<div class="bus-details">
		                
		                <h2 class="job-title">${bus.busName}</h2>
		                <p class="description"><span>${bus.description}</span></p>
		                <br>
            			<br>
            			<b><h2>Date: ${bus.date}</h2></b>
            			</div>
		                <div class="timing">
		                    <h4>${bus.departureTime}</h4>
		                    <p>|</p>
                			<p>|</p>
                			<p>|</p>
		                    <h4>${bus.reachingTime}</h4>
		                    </div>
		                    <div class="route">
		                    <p>${bus.pickup}</p>
		                    <p>|</p>
                			<p>|</p>
                			<p>|</p>
		                    <p>${bus.destination} </p>
		                </div>
		                
		                <div class="extras">
			                <div class="price">
			                    <h4>NPR ${bus.price}</h4>
			                </div>
			                <div class="price">
                    <h4>Status: ${bus.status}</h4>
                    
                </div>
			            </div>
			        </div>
        
		                <div class="agent">
			                <c:forEach var="agent" items="${AgentMap}">
	                        	<c:if test="${bus.agentUsername eq agent.value.agentID}">
			                    
			                    		<div class="agentname">
			                        		<h3>${agent.value.agentPhoneNumber}</h3>
					                        <p>${agent.value.agentFirstName}</p>
					                        <p>${agent.value.agentLastName} </p>
					                    </div>
		                       	 </c:if>
	                        </c:forEach>
	                    </div>
		                <div class="flex-btn">
		                	<form action="${pageContext.request.contextPath}/ModifyBusDetailsServlet" method="get">
			                	<button type="submit"><img src="<%=contextPath + "/resources/other/update.png"%>"></button>
			                	<input type="hidden" name="update_bus_id" value="${bus.busID}">
		                	</form>
		                   <form id="deleteForm-${bus.busID}" action="${pageContext.request.contextPath}/ModifyBusDetailsServlet" method="post">
						    <button type="button" onclick="confirmDelete('${bus.busName}', '${bus.busID}')">
						        <img src="<%=contextPath + "/resources/other/delete.png"%>">
						    </button>
						    <input type="hidden" name="delete_bus_id" value="${bus.busID}">
						</form>
   
		                </div>	
		            </div>
				</c:forEach>
				
				<!-- Add button (only for mentor) -->
				<a href="<%=contextPath + StringUtils.PAGE_URL_ADD%>">
				<div class = "card">
	                <div class="box-add">
	                    <h3>Add Bus</h3>
	                    <p></p>            
	                </div>
	                </div>
	            </a>
	        </c:if>
        
<script>
        
        
        function confirmDelete(busName, busID) {
    		if (confirm("Are you sure you want to delete this bus: " + busName
    				+ "?")) {
    			document.getElementById("deleteForm-" + busID).submit();
    		}else {
                return false;
            }

    	}
    </script>
	
	<jsp:include page="<%=StringUtils.PAGE_URL_FOOTER%>" />


</body>
</html>